...
网络结构 (原版 1.88M 参数):
    Stage 1: ConvBlock(3->16) + ConvBlock(16->32) -> Concat=48 -> SE -> Pool(112x112)
    Stage 2: ConvBlock(48->64) + ConvBlock(64->128) -> Concat=192 + skip(48)=240 -> Pool(56x56)
    Stage 3: ConvBlock(240->128) + ConvBlock(128->256) -> Concat=384 -> SE -> Pool(28x28)
    Stage 4: ConvBlock(384->512) + ConvBlock(512->1024) -> Concat=1536 + skip(384)=1920
    特征聚合: Stage2 中间层(240ch) + Stage4 输出层(1920ch) = 2160ch -> FinalPool -> GAP -> Dropout -> FC
...
class ProposedModel(nn.Module):
    ...
    这是训练版，对应论文中提出的原始架构。四阶段下采样 + 多尺度特征聚合。
